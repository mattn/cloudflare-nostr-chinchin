...
async function doChinchin(request: Request, env: Env): Promise<Response> {
    ...
    let res = await fetch(`https://api.nostr.wine/search?kind=0&pubkey=${mention.pubkey}`)
    let data: { [name: string]: any } = await res.json()
    let user = JSON.parse(data.data[0].content)
